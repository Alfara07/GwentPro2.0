...
public class GameManager : MonoBehaviour
{
    ...
    private void LoadDeck(string decks, GameObject deckPos, Transform positions, Deck player, int TriggerPlayer)
    {
        ...
        foreach (string cart in cartas)
        {
            ...
            List<AttackClass> attack = new();
            string[] ataques = deck[4].Split('-');
            foreach (string a in ataques)
            {
                if (process.CheckAttackClass(a) != AttackClass.None)
                {
                    attack.Add(process.CheckAttackClass(a));
                }
            }
            ...
            CardAdministrator card = new(deck[0], process.CheckCardClass(deck[1]), deck[2], int.Parse(deck[3]), attack, TriggerPlayer);
